...
def add_user(**kwargs):
    ...
    if check_exists(settings.USER_TABLE, username):
        return -1
    ...
    return {'status': ret, 'logger': kwargs.get('logger'), 'msg': msg}
...
def check_exists(table, unique):
    ...
    return os.path.exists(
        os.path.join(settings.DATABASE['tables'][table]['file_path'], "%s.json" % utils.hash_md5(unique)))
...
def deactivate_user(**kwargs):
    ...
    msg = "管理员修改了用户ID %s 为禁用状态" % entry_id
    return {'status': ret, 'logger': kwargs.get('logger'), 'msg': msg}
...
def update_product(**kwargs):
    ...
    if not records:
        print('没有找到结果')
        return -1
    entry_id = input("请输入商品ID:>>").strip()
    price = input("请重新输入单价:>>").strip()
    number = input("请重新输入数量:>>").strip()
    if not price.isdigit() or not number.isdigit():
        return -2
    ret = db_handler.data_api('update', settings.USER_TABLE, **{'set': {'price': float(price), 'number': int(number)},
...
def add_product(**kwargs):
    ...
    :return: 
    """
    title = input("请输入商品名:>>").strip()
    price = input("请输入单价:>>").strip()
    number = input("请输入数量:>>").strip()

    if check_exists(settings.PRODUCT_TABLE, title):
        return -1
